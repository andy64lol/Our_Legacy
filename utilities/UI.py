...
def display_welcome_screen(lang: Any, game_instance: Any):
    ...
    while True:
        ...
        print(f"{Colors.BOLD}{Colors.CYAN}=== {lang.get('main_menu')} ==={Colors.END}")
        ...
        if choice == "3": game_instance.settings_welcome()
        if choice == "4": game_instance.mods_welcome()
        if choice == "5":
...
def display_main_menu(lang: Any, player: Any, area_name: str, menu_max: str):
    ...
    time_str = lang.get("current_time", hour=f"{display_hour:02d}:{display_minute:02d}")
    ...
    if player.current_area == "your_land":
        print(f"{Colors.CYAN}15.{Colors.END} {lang.get('pet_shop', 'Pet Shop')}")
    print(f"{Colors.CYAN}16.{Colors.END} {lang.get('settings', 'Settings')}")
    
    if player.current_area == "your_land":
        print(f"{Colors.YELLOW}17.{Colors.END} {lang.get('furnish_home', 'Furnish Home')}")
        print(f"{Colors.YELLOW}18.{Colors.END} {lang.get('build_structures', 'Build Structures')}")
        print(f"{Colors.YELLOW}19.{Colors.END} {lang.get('farm', 'Farm')}")
        print(f"{Colors.YELLOW}20.{Colors.END} {lang.get('training', 'Training')}")
