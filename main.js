...
import { createProgressBar, createSeparator, createSectionHeader, displayWelcomeScreen, displayMainMenu } from './utilities_js/UI.js';
import { visit_specific_shop } from './utilities_js/shop.js';
import { visit_alchemy } from './utilities_js/crafting.js';
import { build_home, build_structures, farm, training } from './utilities_js/building.js';
...
export function getRarityColor(rarity) {
  ...
  return rarityColors.get(rarity?.toLowerCase(), Colors.WHITE);
}
 ...
 */
export async function ask(game, prompt, validChoices = null, allowEmpty = true, caseSensitive = false, suggest = true) {
  ...
  while (true) {
    ...
    let cmpChoices = [];
    if (validChoices) {
      cmpChoices = validChoices.map(c => caseSensitive ? c : c.toLowerCase());
    }
    ...
    if (suggest && cmpChoices.length > 0) {
      ...
      if (close.length > 0) {
        game.print(lang.get("did_you_mean_msg", "Did you mean one of these? {close}").replace("{close}", close.join(', ')));
      } else {
        game.print(lang.get("invalid_input_choices_msg", "Invalid input. Allowed choices: {choices}").replace("{choices}", cmpChoices.join(', ')));
      }
    } else {
      game.print(lang.get("invalid_input_choices_msg", "Invalid input. Allowed choices: {choices}").replace("{choices}", cmpChoices.join(', ')));
    }
...
export class Game {
  constructor(gameInstance = null) {
    ...
    this.lang = new LanguageManager({
      getSetting: (key) => this.settingsManager?.get(key),
      setSetting: (key, value) => this.settingsManager?.set(key, value)
    });
  ...
  async loadModData() {
    ...
    for (const [fileName, attrName] of modDataTypes) {
      ...
      if (modData && Object.keys(modData).length > 0) {
        ...
        if (baseData) {
          ...
          if (fileName === 'dungeons.json') {
            ...
            if (modData.challenge_templates) {
              if (!baseData.challenge_templates) baseData.challenge_templates = {};
              Object.assign(baseData.challenge_templates, modData.challenge_templates);
            }
        ...
        this.print(`  Loaded ${Object.keys(modData).length} entries from mods for ${fileName}`);
      }
    }
  ...
  async playCutscene(cutsceneId) {
    if (!this.cutscenesData[cutsceneId]) {
      this.print(this.lang.get("cutscene_not_found_msg").replace("{cutscene_id}", cutsceneId));
      return;
    }
  ...
  async playCutsceneContent(content) {
    ...
    if (content.choice) {
      ...
      if (choiceKeys.length > 0) {
        ...
        const choice = (await this.ask("Your choice (or press Enter to skip): ")).trim();
  ...
  async settingsWelcome() {
    while (true) {
      ...
      if (choice === "1") {
        ...
        this.print(`${Colors.YELLOW}Note: Changes take effect on game restart.${Colors.END}`);
  ...
  async modsWelcome() {
    while (true) {
      ...
      for (let i = 0; i < modsList.length; i++) {
        ...
        const status = enabled ? `${Colors.GREEN}[ENABLED]${Colors.END}` : `${Colors.RED}[DISABLED]${Colors.END}`;
  ...
  async changeLanguageMenu() {
    ...
    this.print(`${Colors.CYAN}${langs.length + 1}.${Colors.END} ${this.lang.get('back', 'Back')}`);
  ...
  async mainMenu() {
    ...
    const menuOutput = displayMainMenu(this.lang, this.player, areaName, menuMax);
    this.print(menuOutput);
    
    let choice = await this.ask(`${Colors.CYAN}Choose an option (1-${menuMax}): ${Colors.END}`, null, false);
    ...
    const shortcutMap = {
      ...
      'pet_shop': '15',
      'settings': '16', 'lang': '16', 'language': '16',
      'build_home': '17', 'furnish_home': '17',
      'build_land': '18', 'build_structures': '18', 'land': '18',
      'farm': '19',
      'training': '20', 'train': '20',
  ...
  async fightBossMenu() {
    ...
    if (!possibleBosses || possibleBosses.length === 0) {
      this.print(`There are no bosses in ${areaData.name || this.currentArea}.`);
      return;
    }
    ...
    for (let i = 0; i < possibleBosses.length; i++) {
      ...
      if (this.player.bossesKilled && this.player.bossesKilled[bossName]) {
        ...
        try {
          ...
          if (diff < 28800000) { // 8 hours in ms
    ...
    const choice = await this.ask(`Choose a boss (1-${possibleBosses.length}) or Enter to cancel: `);
    if (choice && !isNaN(choice)) {
      const idx = parseInt(choice) - 1;
  ...
  async explore() {
    ...
    this.print(this.lang.get("exploring_area_msg").replace("{area_name}", areaName));
    ...
    if (Math.random() < 0.7) {
      ...
      if (Math.random() < 0.3) {
        ...
        this.print(this.lang.get("found_gold_msg").replace("{gold}", foundGold));
      }
    }
  }
  ...
  updateChallengeProgress(challengeType, value = 1) {
    ...
    for (const challenge of challenges) {
      ...
      if (challenge.type === challengeType) {
        ...
        // Show progress bar
        const bar = createProgressBar(this.challengeProgress[challenge.id], challenge.target, 20, Colors.YELLOW);
        this.print(`${Colors.CYAN}[Challenge Progress] ${challenge.name}: ${bar} ${this.challengeProgress[challenge.id]}/${challenge.target}${Colors.END}`);
  ...
  async viewChallenges() {
    ...
    for (const challenge of challenges) {
      ...
      const completedText = isCompleted ? `${Colors.GREEN}COMPLETED${Colors.END}` : status;
      
      this.print(`\n${Colors.BOLD}${challenge.name}${Colors.END}`);
      this.print(`  ${challenge.description}`);
      this.print(`  Status: ${completedText}`);
      this.print(`  Reward: ${challenge.reward_exp} EXP + ${challenge.reward_gold} Gold`);
    }
  }
  ...
  async useItemInBattle() {
    ...
    const consumables = this.player.inventory.filter(item => 
  ...
  async viewInventory() {
    ...
    const consumables = this.player.inventory.filter(it => 
      (this.itemsData[it] || {}).type === 'consumable'
    );
    
    // Offer equip/unequip options
    const equipable = this.player.inventory.filter(it => 
      (this.itemsData[it] || {}).type in ['weapon', 'armor', 'accessory', 'offhand']
    );
    ...
    if (equipable.length > 0 || consumables.length > 0) {
      ...
      if (consumables.length > 0) {
        this.print(this.lang.get("use_consumable_option", "  C. Use a consumable item"));
      }
      
      const choice = (await this.ask("Choose option (E/U/C) or press Enter to return: ")).toLowerCase();
      ...
      if (choice === 'e' && equipable.length > 0) {
        ...
        for (let i = 0; i < equipable.length; i++) {
          ...
          this.print(`${i + 1}. ${item} - ${(this.itemsData[item] || {}).description || ''}`);
        }
        const sel = await this.ask(`Choose item to equip (1-${equipable.length}) or press Enter: `);
        if (sel && !isNaN(sel)) {
        ...
        const slots = ['weapon', 'armor', 'offhand', 'accessory_1', 'accessory_2', 'accessory_3'];
        for (const slot of slots) {
          this.print(`${slot.charAt(0).toUpperCase() + slot.slice(1)}: ${this.player.equipment[slot] || 'None'}`);
        }
        const slotChoice = await this.ask("Enter slot to unequip or press Enter: ");
        const validSlots = ['weapon', 'armor', 'offhand', 'accessory_1', 'accessory_2', 'accessory_3'];
        ...
        const sel = await this.ask(`Choose item to use (1-${consumables.length}) or press Enter: `);
        if (sel && !isNaN(sel)) {
          const idx = parseInt(sel) - 1;
  ...
  async viewMissions() {
    while (true) {
      ...
      // Active Missions
      const activeMissions = Object.keys(this.missionProgress).filter(mid => 
        !this.missionProgress[mid]?.completed
      );
        ...
        for (let i = 0; i < activeMissions.length; i++) {
          ...
          if (progress.type === 'kill') {
            if (progress.currentCounts) {
              status = Object.entries(progress.targetCounts).map(([t, c]) => 
                `${t}: ${progress.currentCounts[t] || 0}/${c}`
              ).join(", ");
            } else {
          ...
          this.print(`   ${Colors.DARK_GRAY}${mission.description || ''}${Colors.END}`);
        }
      ...
      if (choice === 'B') {
        ...
        if (!isNaN(idxStr)) {
          ...
          if (idx >= 0 && idx < activeMissions.length) {
            ...
            const confirm = (await this.ask(`Are you sure you want to cancel '${mName}'? (y/n): `)).toLowerCase();
            if (confirm === 'y') {
  ...
  async availableMissionsMenu() {
    ...
    while (true) {
      ...
      const availableMissions = Object.keys(this.missionsData).filter(mid => 
      ...
      for (let i = 0; i < currentPageMissions.length; i++) {
        ...
        this.print(`${i + 1}. ${Colors.BOLD}${mission.name || 'Unknown'}${Colors.END}`);
        ...
        if (mission.prerequisites) {
          for (const prereqId of mission.prerequisites) {
            const prereqName = (this.missionsData[prereqId] || {}).name || prereqId;
            const color = this.completedMissions.includes(prereqId) ? Colors.GREEN : Colors.RED;
      ...
      if (totalPages > 1) {
        ...
        if (page < totalPages - 1) this.print(`N. ${this.lang.get('ui_next_page')}`);
      }
  ...
  async acceptMission(missionId) {
    ...
    if (missionType === 'collect' && typeof targetCount === 'object') {
      this.missionProgress[missionId] = {
        currentCounts: Object.fromEntries(Object.keys(targetCount).map(k => [k, 0])),
        ...
        if (progress.type === 'collect') {
          if (progress.currentCounts) {
            for (const item of Object.keys(progress.targetCounts)) {
              const invCount = this.player.inventory.filter(i => i === item).length;
              progress.currentCounts[item] = invCount;
            }
            
            const allCollected = Object.keys(progress.targetCounts).every(item => 
            ...
            const invCount = this.player.inventory.filter(i => i === targetItem).length;
    ...
    for (const [mid, progress] of Object.entries(this.missionProgress)) {
      ...
      if (progress.type === 'kill' && updateType === 'kill') {
        ...
        if (targetEnemy === target.toLowerCase()) {
          ...
          const bar = createProgressBar(progress.currentCount, progress.targetCount, 20, Colors.CYAN);
          this.print(`${Colors.CYAN}[Mission Progress] ${mission.name}: ${bar} ${progress.currentCount}/${progress.targetCount}${Colors.END}`);
        ...
        if (progress.currentCounts && target in progress.currentCounts) {
          ...
          const bar = createProgressBar(progress.currentCounts[target], progress.targetCounts[target], 20, Colors.CYAN);
          this.print(`${Colors.CYAN}[Mission Progress] ${mission.name} - ${target}: ${bar} ${progress.currentCounts[target]}/${progress.targetCounts[target]}${Colors.END}`);
  ...
  completeMission(missionId) {
    if (this.missionProgress[missionId]) {
      ...
      this.print(`${Colors.YELLOW}You can now claim your rewards from the menu.${Colors.END}`);
  ...
  async claimRewards() {
    ...
    const completedMissions = Object.keys(this.missionProgress).filter(mid => 
      this.missionProgress[mid]?.completed
    );
    ...
    for (let i = 0; i < completedMissions.length; i++) {
      ...
      this.print(`${i + 1}. ${mission.name || 'Unknown'} - Exp: ${exp}, Gold: ${gold}, Items: ${items.length > 0 ? items.join(', ') : 'None'}`);
    }
    
    const choice = await this.ask(`Claim rewards for mission (1-${completedMissions.length}) or press Enter to cancel: `);
    if (choice && !isNaN(choice)) {
      const idx = parseInt(choice) - 1;
    ...
    if (availableShops.length > 1) {
      ...
      for (let i = 0; i < availableShops.length; i++) {
        ...
        if (shopId === "housing_shop") {
          ...
          shopName = shopData.name || shopId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        }
  ...
  async visitHousingShopInline() {
    ...
    this.print(`${Colors.YELLOW}Welcome to the Housing Shop! Build your dream home with these items.${Colors.END}`);
    this.print(`\nYour gold: ${Colors.GOLD}${this.player.gold}${Colors.END}`);
    this.print(`Comfort Points: ${Colors.CYAN}${this.player.comfortPoints || 0}${Colors.END}`);
    this.print(`Items owned: ${Colors.MAGENTA}${(this.player.housingOwned || []).length}${Colors.END}`);
    ...
    while (true) {
      ...
      for (let i = 0; i < pageItems.length; i++) {
        ...
        const priceColor = this.player.gold >= price ? Colors.GREEN : Colors.RED;
        ...
        this.print(`   ${desc}`);
        this.print(`   Price: ${priceColor}${price} gold${Colors.END} | Comfort: ${Colors.CYAN}+${comfort}${Colors.END}`);
      }

      this.print(this.lang.get("options_2"));
      this.print(`${Colors.CYAN}1-${pageItems.length}.${Colors.END} Buy/Add Housing Item`);
      ...
      if (!choice) {
        ...
        if (itemIdx >= 0 && itemIdx < pageItems.length) {
          ...
          if ((this.player.housingOwned || []).includes(itemId)) {
            const confirm = (await this.ask(`\n${Colors.YELLOW}You already own this. Buy another copy for ${Colors.GOLD}${price} gold${Colors.YELLOW}? (y/n): ${Colors.END}`)).trim().toLowerCase();
          ...
          if (this.player.gold >= price) {
            ...
            this.player.comfortPoints = (this.player.comfortPoints || 0) + comfort;
            this.print(`\n${Colors.GREEN}✓ Purchased ${Colors.BOLD}${name}${Colors.END}${Colors.GREEN}!${Colors.END}`);
            this.print(`${Colors.CYAN}Comfort points: +${comfort} (Total: ${this.player.comfortPoints})${Colors.END}`);
          } else {
  ...
  async visitTavern() {
    ...
    this.print("Welcome to The Rusty Tankard. Here you can hire companions to join your party.");
    this.print(`Your gold: ${Colors.GOLD}${this.player.gold}${Colors.END}`);
    ...
    while (true) {
      ...
      for (let i = 0; i < pageItems.length; i++) {
        ...
        this.print(`${i + 1}. ${cdata.name || cid} - ${Colors.GOLD}${price} gold${Colors.END}`);
        this.print(`   ${desc}`);
      }
        ...
        if (idx >= 0 && idx < pageItems.length) {
          ...
          if (this.player.gold >= price) {
            if ((this.player.companions || []).length >= 4) {
              this.print("You already have the maximum number of companions (4).");
              continue;
            }
            this.player.gold -= price;
  ...
  async visitMarket() {
    ...
    if (!marketData || !marketData.ok) {
      ...
      this.print(`${Colors.YELLOW}Merchants have travelled to another distant far place!${Colors.END}`);
      this.print(`${Colors.YELLOW}Please wait until the merchants arrive!${Colors.END}`);
      return;
    }
    ...
    const choice = (await this.ask("\nChoose filter (1-5, R) or press Enter to browse all: ")).trim().toUpperCase();
    ...
    while (true) {
      ...
      for (let i = 0; i < pageItems.length; i++) {
        ...
        const priceColor = marketPrice <= (this.player?.gold || 0) ? Colors.GREEN : Colors.RED;

        this.print(`\n${i + 1}. ${rarityColor}${name}${Colors.END} (${itemType})`);
        this.print(`   ${Colors.DARK_GRAY}${desc}${Colors.END}`);
        this.print(`   ${rarityColor}${rarity.charAt(0).toUpperCase() + rarity.slice(1)}${Colors.END} | Level ${levelReq}` + (classReq ? ` | ${Colors.CYAN}${classReq}${Colors.END}` : ""));
        this.print(`   ${priceColor}${marketPrice}${Colors.END} gold (was ${originalPrice})`);
      }
      ...
      if (!actionChoice) {
        ...
        if (subChoice === '1') {
          const itemType = (await this.ask("Enter type: ")).trim().toLowerCase();
          filteredItems = filteredItems.filter(it => (it.type || '').toLowerCase() === itemType);
        } else if (subChoice === '2') {
          const rarity = (await this.ask("Enter rarity: ")).trim().toLowerCase();
          filteredItems = filteredItems.filter(it => (it.rarity || '').toLowerCase() === rarity);
        } else if (subChoice === '3') {
          const classReq = (await this.ask("Enter class: ")).trim();
          filteredItems = filteredItems.filter(it => ((it.requirements || {}).class || '').toLowerCase() === classReq.toLowerCase());
        } else if (subChoice === '4') {
          try {
            const maxPrice = parseInt((await this.ask("Enter max price: ")).trim());
            filteredItems = filteredItems.filter(it => (it.marketPrice || 0) <= maxPrice);
          } catch {
            // Keep filtered items on error
          }
        }
  ...
  async manageCompanions() {
    ...
    while (true) {
      ...
      this.print(`Active companions: ${(this.player.companions || []).length}/4`);

      if (!this.player.companions || this.player.companions.length === 0) {
        this.print("You have no companions yet. Visit the tavern to hire some!");
        return;
      }
      ...
      for (let i = 0; i < this.player.companions.length; i++) {
        ...
        if (compData) {
          ...
          if (compData.healing_bonus) bonuses.push(`+${compData.healing_bonus} Healing`);
      ...
      if (!choice) {
        ...
        if (this.player.companions && this.player.companions.length > 0) {
          try {
            const idx = parseInt((await this.ask(`Dismiss which companion (1-${this.player.companions.length})? `))) - 1;
            if (idx >= 0 && idx < this.player.companions.length) {
              ...
              if (typeof dismissed === 'object') {
                this.print(`${Colors.RED}Dismissed ${dismissed.get('name')}.${Colors.END}`);
              } else {
  ...
  async travel() {
    ...
    const choice = await this.ask(`Travel to (1-${connections.length}) or press Enter to cancel: `);
    if (choice && !isNaN(choice)) {
      const idx = parseInt(choice) - 1;
  ...
  async rest() {
    ...
    this.print(`Current HP: ${Colors.RED}${this.player.hp}/${this.player.maxHp}${Colors.END}`);
    this.print(`Current MP: ${Colors.BLUE}${this.player.mp}/${this.player.maxMp}${Colors.END}`);
    ...
    this.print(`HP restored: ${oldHp} → ${Colors.GREEN}${this.player.hp}${Colors.END}`);
    this.print(`MP restored: ${oldMp} → ${Colors.GREEN}${this.player.mp}${Colors.END}`);
    this.print(`Gold remaining: ${Colors.GOLD}${this.player.gold}${Colors.END}`);
  }
  ...
  async petShop() {
    ...
    while (true) {
      ...
      this.print(createSectionHeader(this.lang.get("pet_shop_header", "PET SHOP")));
      this.print(`${this.lang.get('your_gold', 'Your Gold')}: ${Colors.GOLD}${this.player.gold}g${Colors.END}`);

      // Using localized pet names if possible, else fallback
      let activePetName = "None";
      if (this.player.activePet) {
        activePetName = (this.player.petsData[this.player.activePet] || {}).name || this.player.activePet;
      }

      this.print(`Current Pet: ${Colors.MAGENTA}${activePetName}${Colors.END}\n`);
      ...
      if (choice === '1') {
        ...
        const petInput = (await this.ask("\nEnter pet name to buy or press Enter to cancel: ")).toLowerCase().trim().replace(/ /g, '_');
        if (!petInput) continue;

        if (this.player.petsData[petInput] && !(this.player.petsOwned || []).includes(petInput)) {
        ...
        const sel = await this.ask(`Select pet to activate (1-${this.player.petsOwned.length}) or press Enter: `);
        if (sel && !isNaN(sel)) {
          const idx = parseInt(sel) - 1;
          if (idx >= 0 && idx < this.player.petsOwned.length) {
            this.player.activePet = this.player.petsOwned[idx];
            this.print(`${this.player.petsData[this.player.activePet].name} is now active!`);
            await this.ask("Press Enter to continue...");
          }
        }
      } else if (choice === '3') {
  ...
  async saveGame(filenamePrefix = "") {
    if (this.saveLoadSystem) {
      await this.saveLoadSystem.saveGame(filenamePrefix);
    } else {
  ...
  async loadGame() {
    if (this.saveLoadSystem) {
      await this.saveLoadSystem.loadGame();
    } else {
      this.print("Save system not initialized");
    }
  }
